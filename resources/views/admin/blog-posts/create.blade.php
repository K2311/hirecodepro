...
@section('title', 'Create Blog Post | CodeCraft Studio')

@section('content')
    <div class="page-header mb-4">
        <div class="page-title">
            <h1 class="fw-bold">
                <a href="{{ route('admin.blog-posts.index') }}" class="text-muted decoration-none me-2"><i class="fas fa-arrow-left"></i></a>
                Create Blog Post
            </h1>
        </div>
    </div>

    <form action="{{ route('admin.blog-posts.store') }}" method="POST" id="postForm">
        @csrf
        <div class="row g-4">
            <div class="col-xl-8">
                <!-- Main Content Card -->
                <div class="premium-card border-0 shadow-sm mb-4">
                    <div class="card-body p-4">
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" id="title" class="form-input-premium @error('title') is-invalid @enderror" value="{{ old('title') }}" required placeholder="Enter post title...">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                <span class="input-group-text bg-light border-end-0 rounded-start-pill ps-3 text-muted small" style="border-radius: 12px 0 0 12px;">{{ url('/blog/') }}/</span>
                                <input type="text" name="slug" id="slug" class="form-input-premium rounded-start-0" value="{{ old('slug') }}" placeholder="auto-generated-slug" style="border-radius: 0 12px 12px 0;">
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="excerpt" id="excerpt" rows="3" class="form-input-premium h-auto" placeholder="Brief summary of the post...">{{ old('excerpt') }}</textarea>
                        </div>

                        <div class="mb-0">
                            <label class="form-label small text-uppercase fw-bold text-muted ls-1 mb-2">Post Content</label>
                            <textarea name="content" id="post-content" rows="15" class="form-input-premium">{{ old('content') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="premium-card border-0 shadow-sm">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-4">
                            ...
                            <input type="text" name="meta_title" id="meta_title" class="form-input-premium" value="{{ old('meta_title') }}">
                        </div>
                        <div class="mb-4">
                            <label class="form-label small text-uppercase fw-bold text-muted ls-1 mb-2">Meta Description</label>
                            <textarea name="meta_description" id="meta_description" rows="3" class="form-input-premium h-auto">{{ old('meta_description') }}</textarea>
                        </div>
                        <div class="mb-0">
                            <label class="form-label small text-uppercase fw-bold text-muted ls-1 mb-2">Meta Keywords</label>
                            <input type="text" name="meta_keywords" id="meta_keywords" class="form-input-premium" value="{{ old('meta_keywords') }}" placeholder="comma, separated, keywords">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-4">
                ...
                <div class="premium-card border-0 shadow-sm mb-4">
                    <div class="card-header-premium bg-white py-3 px-4">
                        <h5 class="m-0 fw-bold text-dark"><i class="fas fa-paper-plane me-2 text-primary"></i>Publish Settings</h5>
                    </div>
                    <div class="card-body p-4">
                        <div class="mb-4">
                        ...
                        <div id="scheduled_date_container" class="mb-4 d-none">
                            <label class="form-label small text-uppercase fw-bold text-muted ls-1 mb-2">Publish Date</label>
                            <input type="datetime-local" name="published_at" id="published_at" class="form-input-premium" value="{{ old('published_at') }}">
                        </div>

                        <div class="form-check form-switch mb-4 d-flex align-items-center gap-3 ps-0">
                            <input class="form-check-input ms-0" type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} style="width: 2.8rem; height: 1.4rem;">
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary rounded-pill py-3 fw-bold shadow-sm">Save Blog Post</button>
                            <a href="{{ route('admin.blog-posts.index') }}" class="btn btn-outline-secondary rounded-pill py-2">Discard Changes</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="premium-card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body p-4">
                        <div class="select-wrapper mb-2">
                            <select name="category_id" class="form-select-premium @error('category_id') is-invalid @enderror" required>
                        ...
                        @error('category_id') <div class="invalid-feedback d-block small mt-1">{{ $message }}</div> @enderror
                        <div class="mt-3">
                            <a href="{{ route('admin.blog-categories.index') }}" class="text-primary small text-decoration-none fw-bold">
                ...
                <div class="premium-card border-0 shadow-sm">
                    ...
                    <div class="card-body p-4 text-center">
                        <div id="cover-preview" class="image-upload-preview mb-3 shadow-sm" onclick="document.getElementById('cover-upload').click()">
                            <div class="preview-placeholder">
                                <i class="fas fa-cloud-upload-alt fa-3x mb-3 opacity-25"></i>
                                <p class="mb-0 fw-bold text-muted small">Drop featured image here or click to browse</p>
                            </div>
                        </div>
                        <input type="file" id="cover-upload" hidden accept="image/*">
                        <input type="hidden" name="cover_image_url" id="cover_image_url">
                        <p class="text-muted small mb-0 mt-3">Recommended: 1200x630px (JPG/PNG)</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <style>
        ...
        .form-input-premium, .form-select-premium { width: 100%; height: 48px; padding: 10px 16px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; font-size: 0.95rem; display: block; outline: none; transition: all 0.2s; }
        ...
        .form-input-premium:focus { border-color: var(--primary); box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1); }
        
        .select-wrapper { position: relative; }
        .select-icon { position: absolute; right: 16px; top: 50%; transform: translateY(-50%); color: #94a3b8; pointer-events: none; opacity: 0.7; }

        .image-upload-preview { height: 220px; border: 2px dashed #e2e8f0; border-radius: 16px; display: flex; align-items: center; justify-content: center; overflow: hidden; cursor: pointer; background: #f8fafc; transition: all 0.3s ease; }
        body.dark-mode .image-upload-preview { border-color: var(--dark-border); background: rgba(255, 255, 255, 0.02); }
        .image-upload-preview:hover { border-color: var(--primary); background: rgba(59, 130, 246, 0.05); }
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                ClassicEditor.create(document.querySelector('#post-content'), {
                    ...
                    toolbar: { items: ['heading', '|', 'bold', 'italic', 'underline', 'strikethrough', '|', 'bulletedList', 'numberedList', '|', 'outdent', 'indent', '|', 'imageUpload', 'blockQuote', 'insertTable', 'mediaEmbed', '|', 'undo', 'redo', '|', 'code', 'htmlEmbed', 'sourceEditing'] }
                ...
                titleInput.onblur = function () {
                    if (slugInput.value === '') {
                        slugInput.value = this.value.toLowerCase().replace(/[^\w\s-]/g, '').replace(/[\s_-]+/g, '-').replace(/^-+|-+$/g, '');
                    }
                };
                ...
                document.getElementById('cover-upload').onchange = function (e) {
                    ...
                    fetch('{{ route("admin.blog-posts.upload-media") }}', { method: 'POST', body: formData })
                        ...
                        }).catch(err => alert('Upload error occurred'));
                };
            });
        </script>
    @endpush
