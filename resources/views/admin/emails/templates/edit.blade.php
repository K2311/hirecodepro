...
@section('title', 'Edit Email Template | CodeCraft Studio')
    ...
    <div class="page-header">
        <div class="page-title">
            <a href="{{ route('admin.emails.templates') }}" class="back-link"><i class="fas fa-arrow-left"></i> Back to
                Templates</a>
            <h1>Edit Email Template: {{ $template->name }}</h1>
        </div>
    </div>

    <form action="{{ route('admin.emails.templates.update', $template->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-8">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="body_text" id="body_text" rows="8"
                                ...
                                placeholder="Hello @{{ $user_name }}...">{{ old('body_text', $template->body_text) }}</textarea>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <select name="category" id="category"
                                ...
                                <option value="onboarding" {{ $template->category == 'onboarding' ? 'selected' : '' }}>
                                    Onboarding</option>
                                <option value="transactions" {{ $template->category == 'transactions' ? 'selected' : '' }}>
                                    Transactions</option>
                                <option value="marketing" {{ $template->category == 'marketing' ? 'selected' : '' }}>Marketing
                                </option>
                                <option value="support" {{ $template->category == 'support' ? 'selected' : '' }}>Support
                                </option>
                                <option value="general" {{ $template->category == 'general' ? 'selected' : '' }}>General
                                </option>
                            </select>
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-large">Update Template</button>
